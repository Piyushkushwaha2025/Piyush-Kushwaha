...
import { Monitor, Smartphone, Play, Video, LayoutTemplate, Share2, Star, Sparkles, Image as ImageIcon, ChevronRight, Download, Github, Linkedin, Instagram, Mail, MessageCircle, MapPin, Palette, Bot, X, Send } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e) => {
      const isInteractive = 
        ...
        e.target.closest('a') ||
        e.target.classList.contains('interactive-cursor');
...
const TiltCard = React.forwardRef(({ children, className, style: externalStyle }, ref) => {
  ...
  const handleMouseMove = (e) => {
    ...
    // Increased rotation angles for more extreme 3D pop
    ...
    // 3D Glass Glare Calculation
    const angle = Math.atan2(y - centerY, x - centerX) * (180 / Math.PI) - 90;
    const distance = Math.sqrt(Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2));
    const opacity = Math.min(distance / centerX, 0.4); // Max 40% opacity
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Optimized particle count for connecting lines
    ...
    const animate = () => {
      ...
      stars.forEach((star, i) => {
        ...
        for (let j = i + 1; j < stars.length; j++) {
          ...
          // Draw line if particles are close in 3D projection
          if (dist < 120) {
  ...
  return (
    <div className="min-h-screen bg-black text-neutral-300 font-sans selection:bg-neutral-800 selection:text-white relative overflow-hidden">
      ...
      {/* Dynamic CSS Variables & Global Typography Utilities */}
      <style>{`
        :root {
        ...
        /* 3D Holographic Orbit Animation */
        @keyframes spin-3d {
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/50 backdrop-blur-xl border-b border-white/5 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-5 flex justify-between items-center">
          ...
          <button className="px-6 py-2.5 rounded-full font-bold text-black interactive-cursor text-sm tracking-wider uppercase transition-transform hover:scale-105" style={{ backgroundColor: 'var(--color-accent)' }}>
            Hire Me
          </button>
        </div>
      </nav>
      ...
      <section id="home" className="relative min-h-screen flex items-center pt-24 overflow-hidden z-10">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid lg:grid-cols-2 gap-12 items-center w-full">
          ...
          {/* Left Content (Upgraded Typography) */}
          <div className="flex flex-col items-start z-20">
            ...
            <div className="flex flex-wrap gap-4 mb-12">
              <button className="px-8 py-4 rounded-full font-bold text-white shadow-lg hover:shadow-[0_0_30px_rgba(var(--color-accent-rgb),0.4)] hover:-translate-y-1 transition-all duration-300 interactive-cursor text-sm tracking-wider uppercase" style={{ backgroundColor: 'var(--color-accent)' }}>
                Start a Project
              </button>
              <button className="px-8 py-4 rounded-full font-bold text-white bg-transparent border-2 border-neutral-800 hover:border-white transition-all duration-300 interactive-cursor flex items-center gap-2 text-sm tracking-wider uppercase">
                Explore Work
              </button>
            </div>
            ...
            <div className="flex items-center gap-6">
              {[Github, Linkedin, Instagram, Mail].map((Icon, i) => (
                <a key={i} href="#" className="text-neutral-500 hover:text-white transition-colors interactive-cursor">
                  <Icon size={24} />
                </a>
              ))}
            </div>
          ...
          <div className="relative flex justify-center items-center mt-12 lg:mt-0 z-20">
            ...
            <TiltCard className="relative w-72 h-[26rem] md:w-96 md:h-[36rem] rounded-[2.5rem] border border-neutral-800 bg-[#0a0a0a] shadow-2xl z-10 group interactive-cursor transform-style-3d overflow-visible">
              ...
              {/* 3D Floating Badges */}
              {[
                { text: 'React', style: { top: '10%', right: '-10%' }, delay: '0s' },
                { text: 'Design', style: { bottom: '20%', right: '-5%' }, delay: '0.2s' },
                { text: 'Python', style: { bottom: '10%', left: '-10%' }, delay: '0.4s' },
                { text: 'AI Gen', style: { top: '20%', left: '-5%' }, delay: '0.6s' },
              ].map((badge, i) => (
                <div 
                  key={i}
      ...
      {/* About Section (Upgraded Typography) */}
              ...
              <TiltCard key={i} className="group relative rounded-[2rem] overflow-hidden bg-[#0a0a0a] border border-neutral-900 interactive-cursor transform-style-3d">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4 backdrop-blur-sm pointer-events-none">
                    <button className="p-4 bg-white text-black rounded-full shadow-xl" style={{ transform: 'translateZ(40px)' }}>
      ...
      <footer className="py-8 border-t border-neutral-900 bg-black relative z-10 text-center">
        <p className="text-neutral-500 text-sm font-semibold tracking-wider">
          &copy; 2026 <span className="text-white">PIYUSH KUSHWAHA</span>. ALL SYSTEMS ONLINE.
        </p>
      </footer>
      
      <AIAssistant activeTheme={activeTheme} />
    </div>
  );
}